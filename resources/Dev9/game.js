...
let carLocationRight3 = [];
let carLocationLeft3 = [];
let worldColor;
let gridSize;
let colorG1;
let colorG2;
...
PS.init = function (system, options) {
	...
	if (level == 1) {
		...
		PS.statusText("Level 2: Press Enter to Restart");
		...
		//Create World
		makeRoad(25, 0, 1, false);
		makeRoad(17, 1, 1, false);
		makePond(16, 2);
		...
		//Create Cars
		createCar(0, roadLocations[0] + 2, true, 3, colorG1); // Right
		createCar(gridSize.width + 2, roadLocations[0] - 2, false, 4, colorG2); // Left
		createCar(0, roadLocations[1] + 2, true, 1, colorG3); // Right
...
let myCounterRight3 = 2;
let myTimerIDRight3 = -1;

//Move car2 right
function moveCarRight3() {
	if (myCounterRight3 > 0) {
		myCounterRight3 -= 1; // decrement counter
	} else {
		myCounterRight3 = 3;
		PS.timerStop(myTimerIDRight3); // stop timer
		myTimerIDRight3 = -1;
		deleteCar(carLocationRight3[0], carLocationRight3[1]);
		createCar(carLocationRight3[0] + 1, carLocationRight3[1], true, 5, colorG4);
		if (carLocationRight3[0] - 2 > gridSize.width) {
			createCar(-2, carLocationRight3[1], true, 5, colorG1);
		}
	}

};

let myCounterLeft3 = 2;
let myTimerIDLeft3 = -1;

//Move car2 left
function moveCarLeft3() {
	if (myCounterLeft3 > 0) {
		myCounterLeft3 -= 1; // decrement counter
	} else {
		myCounterLeft3 = 3;
		PS.timerStop(myTimerIDLeft3); // stop timer
		myTimerIDLeft3 = -1;
		deleteCar(carLocationLeft3[0], carLocationLeft3[1]);
		createCar(carLocationLeft3[0] - 1, carLocationLeft3[1], false, 6, colorG2);
		if (carLocationLeft3[0] + 2 < 0) {
			createCar(gridSize.width + 2, carLocationLeft3[1], false, 6, colorG3);
		}
	}
};
...
function createCar(x, y, right, id, colorG) {
	...
	if (myTimerIDRight3 == -1 && right && id == 5) { // Only start a new timer if one isn't already running
		myTimerIDRight3 = PS.timerStart(1.2, moveCarRight3);
	}
	if (myTimerIDLeft3 == -1 && !right && id == 6) { // Only start a new timer if one isn't already running
		myTimerIDLeft3 = PS.timerStart(1.2, moveCarLeft3);
	}
	...
	if (id == 1) {
		...
		carLocationRight3[0] = x;
		carLocationRight3[1] = y;
	} else if (id == 6) {
		carLocationLeft3[0] = x;
		carLocationLeft3[1] = y;
	}
...
function checkHit() {
	...
	if (chickenLocation[0] > 13 && chickenLocation[0] < 20 && chickenLocation[1] < 5) {
		if(level == 3){
			PS.statusText("You got to the pond! Press Enter to Restart");
		}
		if (!gameOver) {
